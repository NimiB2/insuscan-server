...
/**
 * HTTP client for Google's Gemini generative AI API. Wraps the REST endpoint at
 * generativelanguage.googleapis.com and exposes typed helpers for the vision
 * and flash models used across the InsuScan pipeline.
 */
@Component
public class GeminiApiClient {
	...
	public GeminiApiClient(WebClient.Builder webClientBuilder, ObjectMapper objectMapper, ApiLogger apiLogger) {
	...
	public String getKeyPreview() {
		return (geminiApiKey != null && geminiApiKey.length() > 5) ? geminiApiKey.substring(0, 5) : "N/A";
	}
	...
	public String callModel(String model, String prompt, String base64Image, String systemPrompt, double temperature) {
		...
		try {
			String response = webClient.post().uri(uri).header("Content-Type", "application/json")
	...
	private Map<String, Object> buildRequestBody(String prompt, String base64Image, String systemPrompt,
			double temperature) {
		...
		if (base64Image != null && !base64Image.isBlank()) {
			parts.add(Map.of("inlineData", Map.of("mimeType", "image/jpeg", "data", base64Image)));
		}
	...
	private String extractTextFromResponse(String rawResponse) {
		...
		try {
			...
			if (candidates != null && candidates.isArray() && !candidates.isEmpty()) {
				...
				log.info("[GEMINI] finishReason={} usage={}", finishReason != null ? finishReason.asText() : "none",
