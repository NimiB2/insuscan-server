...
/**
 * Stage 2.5 — Detects camera tilt from the top image and applies perspective correction when tilt is significant.
 * Can be effectively disabled by setting {@link #SKIP_THRESHOLD_DEG} to 90.0f, in which case tilt estimation is bypassed and the context is marked as not corrected.
 * When enabled, tilt above {@link #WARNING_THRESHOLD_DEG} adds a medium-severity warning even when correction is applied.
 */
@Service
public class PerspectiveCorrectionService {
    ...
    /** Minimum tilt (in degrees) required to trigger a correction. When set to 90.0f the service is effectively disabled. */
//  private static final float SKIP_THRESHOLD_DEG    = 5.0f;
    private static final float SKIP_THRESHOLD_DEG = 90.0f;

    /** Tilt above this value (in degrees) adds a warning about reduced accuracy even after correction. */
    ...
    private boolean applyCorrection(PipelineContext ctx, TiltResult tilt) {
        try {
            ...
            BufferedImage corrected = warpPerspective(image, cxPx, cyPx, aPx, bPx, tilt.rotationDeg);
    ...
    private BufferedImage warpPerspective(BufferedImage src,
                                          float cx, float cy,
                                          float a, float b,
                                          float rotDeg) {
        int size = (int) (a * 2);
