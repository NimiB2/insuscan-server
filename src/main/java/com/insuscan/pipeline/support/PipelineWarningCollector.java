...
/**
 * Collects and manages warnings throughout the pipeline.
 * Provides convenience methods for common warning patterns.
 */
@Component
public class PipelineWarningCollector {
    ...
    public void calibrationFailed(PipelineContext ctx, String reason) {
        add(ctx, PipelineWarning.critical(
            STAGE_CALIBRATION, "CALIBRATION_FAILED",
    ...
    public void referenceObjectNotFound(PipelineContext ctx, String imageView) {
        add(ctx, PipelineWarning.high(
            STAGE_CALIBRATION, "REF_NOT_FOUND_IN_" + imageView.toUpperCase(),
    ...
    public void calibrationRatioMismatch(PipelineContext ctx, float ratioTop, float ratioSide, float gapPct) {
        add(ctx, PipelineWarning.high(
            STAGE_CALIBRATION,
            "RATIO_MISMATCH",
            String.format("Calibration ratio gap %.1f%% (top=%.4f, side=%.4f). " +
    ...
    public void heightExceedsPlateDepth(PipelineContext ctx, String foodName, float heightCm, float depthCm) {
        add(ctx, PipelineWarning.medium(
            STAGE_FOOD_HEIGHT,
            "HEIGHT_EXCEEDS_DEPTH",
            String.format("'%s' effective height %.1fcm > plate depth %.1fcm. " +
                "Food may be overflowing plate — this is not necessarily an error.", foodName, heightCm, depthCm)
        ));
    }
    ...
    public void heightNotVisible(PipelineContext ctx, String foodName) {
        add(ctx, PipelineWarning.medium(
            STAGE_FOOD_HEIGHT,
            "HEIGHT_NOT_VISIBLE",
            String.format("'%s' is not clearly visible in the side image. Height estimate may be inaccurate.", foodName)
        ));
    }
