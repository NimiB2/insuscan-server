...
public class FoodSearchAiServiceImpl implements FoodSearchAiService {
    ...
    public FoodSearchAiServiceImpl(WebClient.Builder webClientBuilder,
    ...
    public SearchOptimization optimizeQuery(String query, String language) {
        ...
        try {
            ...
            log.info("[AI-SEARCH] Optimized '{}' -> '{}' ({}ms)", 
                    query, optimization.getTranslatedQuery(), elapsed);
    ...
    public List<ScoredFoodResult> rankAndScoreResults(String originalQuery, List<NutritionInfo> results) {
        ...
        try {
            ...
            log.info("[AI-SEARCH] Ranked {} results for query '{}' ({}ms)", 
                    scoredResults.size(), originalQuery, elapsed);
    ...
    private String buildRankingPrompt(String originalQuery, List<NutritionInfo> results) {
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            resultsText.append(String.format("%d. ID:%s | %s | %.1fg carbs\n", 
                    i, info.getFdcId(), info.getFoodName(), info.getCarbsPer100g()));
        }
    ...
    private Map<String, Object> buildRequestBody(String prompt) {
        return Map.of(
                "model", openAiModel,
                "messages", List.of(
                        Map.of("role", "system", "content", "You are a JSON-only response engine. Always return valid JSON."),
    ...
    private SearchOptimization parseOptimizationResponse(String jsonResponse) {
        try {
            ...
            String translatedQuery = root.has("translatedQuery") ? 
                    root.get("translatedQuery").asText() : "";
